...
const SidebarItem = ({ item, pageName, setPageName }: any) => {
      ...
      {/* <li>
        <Link
          ...
          className={`${isItemActive
              ? "bg-[#ebebeb] shadow-[inset_5px_5px_10px_#babecc,inset_-5px_-5px_10px_#ffffff]"
              : "bg-[#ebebeb] shadow-[5px_5px_10px_#babecc,-5px_-5px_10px_#ffffff]"
            } group  relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-black duration-300 ease-in-out hover:shadow-[8px_8px_15px_#babecc,-8px_-8px_15px_#ffffff] hover:text-[#0D426E] dark:bg-meta-4`}
        >
          {item.icon}
          {item.label}
          {item.children && (
  ...
  <Link
    ...
    className={`${isItemActive
      ...
      } group relative flex items-center gap-2.5 rounded-sm px-4 py-2  text-black font-semibold duration-300 ease-in-out hover:bg-gray-100 hover:border-gray-400 hover:text-[#0D426E] dark:bg-meta-4 uppercase hover:bg-white`}
  >
    {item.icon}
    {item.label}
    {item.children && (
